...
describe('enforcePolicy', () => {
  ...
  it('throws CRYPTO_POLICY_VIOLATION for disallowed algorithm', () => {
    expect(() =>
      ...
      expect.objectContaining({ code: ErrorCode.CRYPTO_UNSUPPORTED_ALGORITHM }) as Error,
    );
  });
  ...
  it('throws CRYPTO_KEY_INVALID_STATE for non-ACTIVE key', () => {
    ...
    expect(() =>
      enforcePolicy(DEFAULT_ALGORITHM_POLICY, drafted, 'rsa', 'ENCRYPT', 'TEST'),
  ...
  it('throws CRYPTO_KEY_INVALID_STATE for revoked key', () => {
    ...
    expect(() =>
      enforcePolicy(DEFAULT_ALGORITHM_POLICY, revoked, 'rsa', 'ENCRYPT', 'TEST'),
    ).toThrowError(expect.objectContaining({ code: ErrorCode.CRYPTO_KEY_INVALID_STATE }) as Error);
  });
  ...
  it('throws POLICY_ENVIRONMENT_MISMATCH for wrong environment', () => {
    ...
    expect(() =>
      enforcePolicy(DEFAULT_ALGORITHM_POLICY, testKey, 'rsa', 'ENCRYPT', 'PRODUCTION'),
    ).toThrowError(
      expect.objectContaining({ code: ErrorCode.POLICY_ENVIRONMENT_MISMATCH }) as Error,
    );
  });
  ...
  it('throws CRYPTO_POLICY_VIOLATION for wrong usage', () => {
    ...
    expect(() =>
      enforcePolicy(DEFAULT_ALGORITHM_POLICY, encryptOnly, 'rsa', 'SIGN', 'TEST'),
    ).toThrowError(expect.objectContaining({ code: ErrorCode.CRYPTO_POLICY_VIOLATION }) as Error);
  });
  ...
  describe('forbidden registry', () => {
    ...
    it('forbidden check fires before allowlist (SHA-1 would fail either way, but reason must be "forbidden")', () => {
      try {
        ...
        expect((err as { context: { violatedRule: string } }).context.violatedRule).toBe(
          'forbidden_hash',
        );
      }
    });
  ...
  describe('new key states (SUSPENDED, COMPROMISED, DESTROYED)', () => {
    it('rejects a SUSPENDED key (CRYPTO_KEY_INVALID_STATE)', () => {
      ...
      expect(() =>
        enforcePolicy(DEFAULT_ALGORITHM_POLICY, suspended, 'rsa', 'ENCRYPT', 'TEST'),
      ).toThrowError(
        expect.objectContaining({ code: ErrorCode.CRYPTO_KEY_INVALID_STATE }) as Error,
      );
    });
    ...
    it('rejects a COMPROMISED key (CRYPTO_KEY_INVALID_STATE)', () => {
      ...
      expect(() =>
        enforcePolicy(DEFAULT_ALGORITHM_POLICY, compromised, 'rsa', 'ENCRYPT', 'TEST'),
      ).toThrowError(
        expect.objectContaining({ code: ErrorCode.CRYPTO_KEY_INVALID_STATE }) as Error,
      );
    });
    ...
    it('rejects a DESTROYED key (CRYPTO_KEY_INVALID_STATE)', () => {
      ...
      expect(() =>
        enforcePolicy(DEFAULT_ALGORITHM_POLICY, destroyed, 'rsa', 'ENCRYPT', 'TEST'),
      ).toThrowError(
        expect.objectContaining({ code: ErrorCode.CRYPTO_KEY_INVALID_STATE }) as Error,
      );
    });
