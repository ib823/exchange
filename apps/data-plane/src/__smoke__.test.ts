...
describe('M3.0 install smoke — @sep/data-plane', () => {
  it('imports @aws-sdk/client-s3 (install-only, wired M3.5)', async () => {
  ...
  it('imports @aws-sdk/s3-request-presigner (install-only, wired M3.5)', async () => {
    const mod = await import('@aws-sdk/s3-request-presigner');
    expect(typeof mod.getSignedUrl).toBe('function');
  });

  it('imports clamscan (install-only, wired M3 for malware scanning)', async () => {
    const mod = await import('clamscan');
    // clamscan ships as a class via default export; check module loads.
    expect(mod).toBeTruthy();
  });

  it('imports undici at promoted version (§11 override 7.25.0)', async () => {
