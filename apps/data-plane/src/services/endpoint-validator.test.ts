...
describe('validateDeliveryEndpoint', () => {
  ...
  it.each([
    ...
    expect(() => validateDeliveryEndpoint(ip)).toThrow(
      expect.objectContaining({ code: ErrorCode.DELIVERY_ENDPOINT_BLOCKED }),
    );
  });
  ...
  it('blocks cloud metadata IP 169.254.169.254', () => {
    expect(() => validateDeliveryEndpoint('169.254.169.254')).toThrow(
      expect.objectContaining({ code: ErrorCode.DELIVERY_ENDPOINT_BLOCKED }),
    );
  });

  it('blocks cloud metadata IP 169.254.170.2', () => {
    expect(() => validateDeliveryEndpoint('169.254.170.2')).toThrow(
      expect.objectContaining({ code: ErrorCode.DELIVERY_ENDPOINT_BLOCKED }),
    );
  });
  ...
  it.each(['localhost', 'metadata.google.internal', 'kubernetes.default'])(
    'blocks hostname %s',
    (host) => {
      expect(() => validateDeliveryEndpoint(host)).toThrow(
        expect.objectContaining({ code: ErrorCode.DELIVERY_ENDPOINT_BLOCKED }),
      );
    },
  );
  ...
  it('blocks IPv6 loopback ::1', () => {
    expect(() => validateDeliveryEndpoint('::1')).toThrow(
      expect.objectContaining({ code: ErrorCode.DELIVERY_ENDPOINT_BLOCKED }),
    );
  });

  it('blocks IPv6 link-local fe80:', () => {
    expect(() => validateDeliveryEndpoint('fe80::1')).toThrow(
      expect.objectContaining({ code: ErrorCode.DELIVERY_ENDPOINT_BLOCKED }),
    );
  });
  ...
  it('blocks link-local 169.254.x.x', () => {
    expect(() => validateDeliveryEndpoint('169.254.1.1')).toThrow(
      expect.objectContaining({ code: ErrorCode.DELIVERY_ENDPOINT_BLOCKED }),
    );
  });
  ...
  it('blocks multicast 224.x.x.x', () => {
    expect(() => validateDeliveryEndpoint('224.0.0.1')).toThrow(
      expect.objectContaining({ code: ErrorCode.DELIVERY_ENDPOINT_BLOCKED }),
    );
  });
});
